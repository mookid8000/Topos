...
public class TestMongoDbPositionsManager : MongoFixtureBase
{
    ...
    {
        await _positionManager.SetAsync(new Position("test-topic", 1, 100));
        await _positionManager.SetAsync(new Position("test-topic", 2, 100));
        await _positionManager.SetAsync(new Position("test-topic", 7, -1));
        await _positionManager.SetAsync(new Position("test-topic", 7, (long)int.MaxValue + 5));
    }

    [Test]
    public async Task GetsNothingWhenAskingForPositionsInitially()
    {
        var position = await _positionManager.GetAsync("test-topic", partition: 1);
    ...
    public async Task CanGetSinglePosition()
    {
        ...
        var position = await _positionManager.GetAsync("test-topic", 2);
    ...
    {
        await _positionManager.SetAsync(new Position("test-topic", 2, 100));
        await _positionManager.SetAsync(new Position("test-topic", 2, 101));
        await _positionManager.SetAsync(new Position("test-topic", 2, 110));
